...
test("should be possible to combine two states", () => {
  ...
  const n1TimesN2 = compute([state1, state2], (v1, v2) => v1 * v2);
...
test("should not fire when value is unchanged", () => {
  ...
  const areaState = compute([wState, hState], (w, h) => w * h);
  ...
  expect(log).toEqual(["callback n=12", "callback n=6", "callback n=12"]);
});
...
test("should be possible to combine state with template string", () => {
  ...
  expect(log).toEqual(["a = 1, b = foo", "a = 1, b = bar", "a = 2, b = bar"]);
});
